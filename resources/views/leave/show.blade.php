<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-extrabold text-xl text-slate-900 leading-tight tracking-tight">{{ __('Leave Request Detail') }}</h2>
                <p class="text-xs text-slate-400 font-medium mt-0.5">{{ __('Detailed view of the submitted leave application') }}</p>
            </div>
            <div>
            ...
            <div class="p-6 sm:p-8 bg-slate-50/50 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-md text-xs font-extrabold border uppercase tracking-wider {{ $typeColor }}">
                        {{ __($leaveRequest->type) }}
                    </span>
                </div>
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    {{ __('Submitted') }} {{ $leaveRequest->created_at->diffForHumans() }}
                </div>
            </div>
            ...
            <div class="p-6 sm:p-8 space-y-6">
                ...
                @if($isAdminOrHR)
                    <div class="space-y-2 pb-6 border-b border-slate-100">
                        <h4 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('Applicant') }}</h4>
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 font-semibold mt-0.5">
                                        {{ $leaveRequest->user->employee->employee_id ?? __('No NIK') }} &bull; {{ $leaveRequest->user->employee->department->name ?? __('No Department') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            @if(isset($annualLeave))
                                <div class="bg-brand-50 border border-brand-100 rounded-2xl px-4 py-2.5 text-right flex-shrink-0">
                                    <p class="text-[9px] font-extrabold text-brand-800 uppercase tracking-wider">{{ __('Available Balance') }}</p>
                                    <p class="text-sm font-black text-brand-700 mt-0.5">{{ $annualLeave['Remain'] ?? 0 }} <span class="text-[10px] font-bold text-brand-500/80">{{ __('days') }}</span></p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-6 border-b border-slate-100">
                    <div class="space-y-1">
                        <h4 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('Time Period') }}</h4>
                        <p class="text-sm font-bold text-slate-800">
                            {{ $leaveRequest->start_date->translatedFormat('d M Y') }} — {{ $leaveRequest->end_date->translatedFormat('d M Y') }}
                        </p>
                    </div>
                    <div class="space-y-1">
                        <h4 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('Duration') }}</h4>
                        <p class="text-sm font-bold text-slate-800">
                            {{ $leaveRequest->start_date->diffInDays($leaveRequest->end_date) + 1 }} {{ $leaveRequest->start_date->diffInDays($leaveRequest->end_date) + 1 == 1 ? __('day') : __('days') }}
                        </p>
                    </div>
                </div>
                ...
                <!-- Reason -->
                <div class="space-y-2">
                    <h4 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('Reason / Description') }}</h4>
                    <div class="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm text-slate-700 leading-relaxed font-medium">
                        {{ $leaveRequest->reason ?: __('No reason provided.') }}
                    </div>
                </div>
                ...
                @if($leaveRequest->status === 'pending')
                    <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                        ...
                        @else
                            ...
                            <form action="{{ route('leave-requests.destroy', $leaveRequest) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Are you sure you want to cancel this request?') }}');">
                                @csrf
                                @method('DELETE')
