    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form action="{{ route('employees.store') }}" method="POST" class="space-y-6">
            ...
            <div class="card p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="first_name" class="input" value="{{ old('first_name') }}" required placeholder="e.g. Jane">
                        @error('first_name') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                        <input type="text" name="last_name" class="input" value="{{ old('last_name') }}" placeholder="e.g. Doe">
                        @error('last_name') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email Address *</label>
                        <input type="email" name="email" class="input" value="{{ old('email') }}" required placeholder="jane@example.com">
                        @error('email') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                        <input type="text" name="phone" class="input" value="{{ old('phone') }}" placeholder="+1 (555) 000-0000">
            ...
            <div class="card p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="employee_id" class="input" value="{{ old('employee_id') }}" required placeholder="EMP-001">
            ...
            <div class="flex items-center justify-end space-x-4">
                <a href="{{ route('employees.index') }}" wire:navigate class="text-gray-600 hover:text-gray-900 font-medium text-sm">Cancel</a>
