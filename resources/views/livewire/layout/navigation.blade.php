            ...
            <div x-show="sidebarOpen" @click.away="sidebarOpen = false" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="relative flex w-full max-w-xs flex-1 flex-col bg-white pt-5 pb-4 shadow-xl">
                ...
                <div class="flex flex-shrink-0 items-center px-4">
                    <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center space-x-2">
                        ...
                        <span class="text-xl font-bold text-gray-900 tracking-tight">Employee Portal</span>
                    </a>
                </div>
                <div class="mt-8 h-0 flex-1 overflow-y-auto">
                    <nav class="space-y-1 px-2">
                        ...
                        <a href="{{ route('daily-work-logs.index') }}" @click="sidebarOpen = false" wire:navigate class="{{ request()->routeIs('daily-work-logs.*') ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex items-center px-2 py-2 text-base font-medium rounded-md transition-colors">
                            <svg class="{{ request()->routeIs('daily-work-logs.*') ? 'text-brand-700' : 'text-gray-400 group-hover:text-gray-500' }} mr-4 flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                            </svg>
                        </a>

                        @can('sync attendance')
                            <a href="{{ route('system.api-logs') }}" @click="sidebarOpen = false" wire:navigate class="{{ request()->routeIs('system.api-logs') ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex items-center px-2 py-2 text-base font-medium rounded-md transition-colors">
                                <svg class="{{ request()->routeIs('system.api-logs') ? 'text-brand-700' : 'text-gray-400 group-hover:text-gray-500' }} mr-4 flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <div class="flex flex-grow flex-col overflow-y-auto pt-5 pb-4">
            ...
            <div class="mt-8 flex flex-grow flex-col">
                <nav class="flex-1 space-y-2 px-4 bg-white">
                    ...
                    @can('view daily logs')
                        <a href="{{ route('daily-work-logs.index') }}" wire:navigate class="{{ request()->routeIs('daily-work-logs.*') ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors">
                            <svg class="{{ request()->routeIs('daily-work-logs.*') ? 'text-brand-700' : 'text-gray-400 group-hover:text-gray-500' }} mr-3 flex-shrink-0 h-5 w-5 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                            </svg>
                            Daily Work Logs
                        </a>
                    ...
                    @can('sync attendance')
                        <a href="{{ route('system.api-logs') }}" wire:navigate class="{{ request()->routeIs('system.api-logs') ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors">
                            <svg class="{{ request()->routeIs('system.api-logs') ? 'text-brand-700' : 'text-gray-400 group-hover:text-gray-500' }} mr-3 flex-shrink-0 h-5 w-5 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
                            </svg>
                            API Logs
                        </a>
